import { default as Datum } from "../domain/datum.js";
import DatumFilter from "../domain/datumFilter.js";
import { default as Instruction, CommonInstructionTopicName, } from "../domain/instruction.js";
import { default as InstructionStates, } from "../domain/instructionState.js";
import AuthorizationV2Builder from "../net/authV2.js";
import { default as HttpHeaders, HttpContentType, HttpMethod, } from "../net/httpHeaders.js";
import SolarQueryApi from "../net/solarQueryUrlHelper.js";
 ...
 * toggler.value(1);
 */
export default class ControlToggler {
    #api;
    #auth;
    #queryApi;
    #queryAuth;
    ...
    #getActiveInstruction(data) {
        ...
        if (instruction !== undefined) {
            log.debug("Active instruction %d for node %d found in state %s (set control %s to %s)", instruction.id, this.nodeId, instruction.state, controlId, instruction.parameters[0].value);
    ...
    update() {
        ...
        return Promise.all(reqs)
            ...
            if (newValue != currValue || execInstruction) {
                log.debug("Current node %d control %s value is %s", this.nodeId, this.controlId, newValue);
