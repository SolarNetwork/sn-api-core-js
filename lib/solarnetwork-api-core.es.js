// https://github.com/SolarNetwork/sn-api-core-js Version 1.99.1-dev.0. Copyright 2023 SolarNetwork Foundation.
...
class InstructionUrlHelperMixin extends superclass {
    ...
    queueInstructionsUrl(topic, parameters, nodeIds) {
        const nodes = nodeIds || this.param(DatumFilterKeys.NodeIds);
        let url = this.baseUrl() +
            "/instr/add/" +
            encodeURIComponent(topic) +
            "?nodeIds=" +
...
class UrlHelper {
    ...
    hostUrl() {
        ...
        const host = this.environment.value("proxyHost") ||
            this.environment.value("host");
        const port = +(this.environment.value("proxyPort") ||
 ...
 */
class SolarUserApi extends AuthUrlHelperMixin(AuthTokenUrlHelperMixin(UserDatumAuxiliaryUrlHelperMixin(UserMetadataUrlHelperMixin(UserUrlHelperMixin(SolarUserUrlHelper))))) {
}
...
export { index$2 as Domain, index$1 as Net, index as Util };
