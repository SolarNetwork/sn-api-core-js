...
import Datum from "../domain/datum.js";
...
import { Loader, LoaderDataCallbackFn } from "../net/loader.js";
 ...
 */
export default class DatumLoader
	extends JsonClientSupport<SolarQueryApi, Datum[]>
	implements Loader<Datum[]>
{
	 ...
	 * after _each result page_ that is loaded, passing all results loaded thusfar. The function
	 * will be passed a second `boolean` argument that will be set to `true` only on the last page
	...
	#loadData(page: Pagination, q?: Queue): void {
		...
		const handler = (error?: Error, data?: QueryResultsData) => {
			...
			const nextOffset = offsetExtractor(data as QueryResultsData, page);
			...
			if (!q) {
				this.#results =
					this.#results === undefined
						? dataArray
						: this.#results.concat(dataArray as Datum[]);
			}
 ...
 */
function offsetExtractor(data: QueryResultsData, page: Pagination): number {
