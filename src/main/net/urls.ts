			...
			if (pair.length === 2) {
				...
				if (params[k]) {
					if (!Array.isArray(params[k])) {
						params[k] = [params[k] as string]; // turn into array;
					}
					(params[k] as string[]).push(v);
...
function urlQueryEncode(
	...
	encoderFn?: (uriComponent: string | number | boolean) => string
) {
	let result = "";
	const encoder = encoderFn || encodeURIComponent;
	function handleValue(k: string, v: any) {
		if (result.length) {
			result += "&";
		}
