...
function populateProperties(
	...
	let val, name, valLen;
	for (
		let i = 0, iMax = Math.min(names.length, values.length);
		i < iMax;
		i += 1
	) {
		val = values[i];
...
class StreamAggregateDatum implements StreamedDatum {
	...
	constructor(
		streamId: string,
		ts: Array<Date | number | string>,
		iProps?: InstantaneousPropertyStatistics[],
		aProps?: AccumulatingPropertyStatistics[],
		sProps?: string[],
	 ...
	 */
	get date(): Date {
		return this.ts.length ? this.ts[0] : new Date(undefined as any);
	}
	...
	): number[] | string[] | Set<string> | undefined {
	...
	toJsonObject(registry?: DatumStreamMetadataRegistry): any[] {
		const result = [
			registry instanceof DatumStreamMetadataRegistry
				...
				: this.streamId,
			this.ts.map((e) =>
				e instanceof Date && !isNaN(e.getTime()) ? e.getTime() : null
			),
		];
	...
	static fromJsonEncoding(
		json: string,
		meta: DatumStreamMetadata | DatumStreamMetadataRegistry
	): StreamAggregateDatum | undefined {
	...
	static fromJsonObject(
		...
		let i: number,
			len: number,
			m: DatumStreamMetadata | undefined,
			iProps: InstantaneousPropertyStatistics[] | undefined,
			aProps: AccumulatingPropertyStatistics[] | undefined,
			sProps: string[] | undefined,
