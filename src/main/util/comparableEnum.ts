 ...
 * @abstract
 */
abstract class ComparableEnum
	extends Enum
	implements Comparable<ComparableEnum>
{
	 ...
	 */
	static minimumEnumSet<T extends ComparableEnum>(
		minEnum?: T,
		cache?: Map<string, Set<T>>
	): Set<T> | undefined {
		if (!minEnum) {
			return undefined;
		}
		...
		result = new Set<T>();
		for (const agg of (minEnum.constructor as any).enumValues() as T[]) {
