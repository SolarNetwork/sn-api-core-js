...
} from "../../main/tool/controlToggler.js";
...
test.serial("setValue:httpError:notJson", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	http.intercept({
		path: "/solaruser/api/v1/sec/instr/add/SetControlParameter",
	...
	t.regex(
		error.message,
		/^SyntaxError:/,
		"error message from malformed JSON"
	);
	t.is(
		toggler.hasPendingStateChange,
		false,
...
test.serial("update:error:http500", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	...
	const callbackErrors: Array<Error> = [];
	const callback: ControlCallbackFn = function (error) {
		t.truthy(error, "error reported");
	...
	toggler.callback = callback;
	const result = await toggler.update();
	t.falsy(result, "No result available from HTTP error");
	t.regex(callbackErrors[0].message, /^HTTP 500/);
});
...
test.serial(
	"update:noPendingLastKnownQueued:noRecentDatum:noParameters",
	async (t) => {
		// GIVEN
		const http = t.context.agent.get(BASE_URL);
		const results = [
		...
		// THEN
		t.false(toggler.hasPendingStateChange, "no pending change");
		t.is(result, undefined, "no resolved value from updated instruction");
		t.is(
			toggler.value(),
			undefined,
			"no current value from updated instruction"
		);
		t.deepEqual(callbackValues, [undefined, undefined], "callback invoked");
	}
);

test.serial(
	"update:noPendingLastKnownQueued:statusNewerThanInstruction",
	async (t) => {
		// GIVEN
		const http = t.context.agent.get(BASE_URL);
		const results = [
			...
			// most recent value updated more recently than instruction (another instruction?)
			{
				totalResults: 1,
				startingOffset: 0,
				returnedResultCount: 1,
		...
		// THEN
		t.false(toggler.hasPendingStateChange, "no pending change");
		t.is(result, 3, "resolved value is from view instruction");
		t.is(toggler.value(), 3, "current value is from view instruction");
		t.deepEqual(callbackValues, [undefined, 3], "callback invoked");
	}
);

test.serial("update:noPendingLastKnownQueued:missingParameters", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// set value 1
	...
	t.deepEqual(callbackValues, [undefined, 0], "callback invoked");
});
...
test.serial("update:activePending:noParameters", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// most recent value, still at 0
	...
	const callback: ControlCallbackFn = function (error) {
		...
		t.false(
			this.hasPendingStateChange,
			"first callback has no pending change because Instruction does not have matching parameters"
		);
		callbackValues.push(this.value());
	};
	...
	// THEN
	t.false(toggler.hasPendingStateChange, "has no pending change");
	t.is(result, 0, "resolved value is from most recent datum");
	t.is(toggler.value(), 0, "current value is from most recent datum");
	t.deepEqual(callbackValues, [0], "callback invoked");
});

test.serial("update:activePending:missindDates", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// most recent value, still at 0
...
test.serial("update:pending:mostRecentNoVal:noPending", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// set control value
	...
	http.intercept({
		path: "/solaruser/api/v1/sec/instr/add/SetControlParameter",
		method: "POST",
		body: "nodeId=123&parameters%5B0%5D.name=test-control&parameters%5B0%5D.value=2",
		headers: {
			accept: "application/json",
			"content-type": "application/x-www-form-urlencoded; charset=UTF-8",
			authorization: AUTH_POST_REGEX,
	...
	//await toggler.value(2);
	const result = await toggler.update();

	// THEN
	t.is(result, undefined, "resolved value not defined");
	t.is(toggler.value(), undefined, "current value is undefined");
	t.deepEqual(callbackValues, [], "callback invoked");
});
...
test.serial("start:defaultDelay", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// most recent value
...
test.serial("stop", async (t) => {
	...
	t.is(callbackValues.length >= 2, true, "At least 2 callbacks");
	t.deepEqual(
		callbackValues.slice(0, 2),
		[1, 2],
		"callback invoked for each update"
	);
});

test.serial("start:setValue", async (t) => {
	// GIVEN
	const http = t.context.agent.get(BASE_URL);
	const results = [
		// most recent value
	...
	await timeout(20);
	const result = await toggler.value(2);
	...
	t.is(toggler.value(), 1, "most recent datum value returned");
	t.is(toggler.hasPendingStateChange, true, "pending state change to 2");
	t.deepEqual(result, results[1], "SetControlParameter instruction returned");
	t.deepEqual(callbackValues, [1, 1], "Last known values given to callback");
});
